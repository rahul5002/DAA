...
int main() 
{
    ...
    int num_tests = 3;
    printf("Time Comparison of Fractional and 0/1 Knapsack Algorithms\n");
    printf("-------------------------------------------------------\n");
    printf("%-10s %-12s %-15s %-15s\n", "n", "Capacity", "Fractional (s)", "0/1 Knapsack (s)");
    printf("-------------------------------------------------------\n");
    for (int t = 0; t < num_tests; t++) 
    {
        ...
        for (int i = 0; i < 5; i++) 
        {
            fractional_knapsack(values, weights, n, capacity, &total_value, &selected_count, selected);
        }
        double frac_time = (double)(clock() - start) / CLOCKS_PER_SEC / 5;
        start = clock();
        for (int i = 0; i < 5; i++) 
        {
            zero_one_knapsack(values, weights, n, capacity, selected);
        }
        double zero_one_time = (double)(clock() - start) / CLOCKS_PER_SEC / 5;
        printf("%-10d %-12d %-15.6f %-15.6f\n", n, capacity, frac_time, zero_one_time);
        free(values);
        free(weights);
        free(selected);
    }
